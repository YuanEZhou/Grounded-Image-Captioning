...
class AttModel(CaptionModel):
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        if self.opt.att_supervise:
            att_weights = fc_feats.new_zeros(batch_size, seq.size(1) - 1, att_feats.size(1))
        ...
        for i in range(seq.size(1) - 1):
            ...
            if self.opt.att_supervise:
                output, state, att_weight = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)
            else:
    ...
    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state):
        ...
        if self.opt.att_supervise:
            output, state, att_weight = self.core(xt, fc_feats, att_feats, p_att_feats, state, att_masks)
        else:
    ...
    def _sample(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        if self.opt.att_supervise:
            att_weights = fc_feats.new_zeros(batch_size, self.seq_length, att_feats.size(1))
        for t in range(self.seq_length + 1):
            if t == 0: # input <bos>
                it = fc_feats.new_zeros(batch_size, dtype=torch.long)

            if self.opt.att_supervise:
                logprobs, state, att_weight = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)
            else:
            ...
            if unfinished.sum() == 0:
                break
        if self.opt.att_supervise:
            return seq, seqLogprobs, att_weights
        else:
...
class TopDownCore(nn.Module):
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None):
        ...
        if self.opt.att_supervise:
            att, att_weight = self.attention(h_att, att_feats, p_att_feats, att_masks)
        else:
...
class Attention(nn.Module):
    ...
    def forward(self, h, att_feats, p_att_feats, att_masks=None):
        ...
        # Sparse attention.
        # pdb.set_trace()
        # topk = torch.topk(dot,5,dim=-1)[0][:,-1].view(-1,1)
        # mask=dot<topk
        # dot=dot.masked_fill(mask,float('-inf'))
        #add temperature
        # dot = dot/0.05
        # weight = F.gumbel_softmax(dot, tau=1, hard=True)
        #======================================================================================
