...
def get_score_attn_for_one_pair_vse(images, captions, cap_lens, opt):
    """
    Images: (N, d) matrix of images
    Captions: (N, d) matrix of captions
    CapLens: (N) array of caption lengths
    """
    #note! images and captions features are l2 normed
    similarity = torch.sum(images * captions, dim=-1).cpu().numpy()
    return similarity